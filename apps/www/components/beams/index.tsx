...
function extendMaterial<T extends THREE.Material = THREE.Material>(
  ...
  if ('roughness' in defaults) {
    ...
    if (roughness) roughness.value = defaults.roughness ?? roughness.value;
  }
  if ('metalness' in defaults) {
    const metalness = uniforms.metalness;
    if (metalness) metalness.value = defaults.metalness ?? metalness.value;
  }
  if ('envMap' in defaults) {
    const envMap = uniforms.envMap;
    if (envMap) envMap.value = defaults.envMap ?? envMap.value;
  }
  if ('envMapIntensity' in defaults) {
    const envMapIntensity = uniforms.envMapIntensity;
    if (envMapIntensity) envMapIntensity.value = defaults.envMapIntensity ?? envMapIntensity.value;
  }
...
>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    ...
    if (!current) return;
    const uniforms = current.material.uniforms as Record<string, THREE.IUniform | undefined>;
